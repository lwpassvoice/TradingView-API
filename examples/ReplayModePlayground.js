...
async function step() {
  ...
  console.log(
    "Next ->",
    period.time,
    new Date(period.time * 1000).toLocaleString(),
    times.length
  );

  await chart.replayStep(step);
  step();
}

chart.onUpdate(async () => {
  console.log('onUpdate len', chart.periods.length);
  console.log('onUpdate periods', chart.periods);
});
